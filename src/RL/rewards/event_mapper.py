...
class RewardMapper:
    ...
    def map_event(self, event: Event) -> RewardLedger:
        ...
        # Key Hunt structured events are authoritative for key pickups.
        # Supplemental console pickup messages must not add a second
        # generic item reward.
        if (
            event.type == "item_pickup"
            and getattr(
                self.current_mode,
                "value",
                None,
            ...
            item_text = " ".join(
                (
                    str(event.data.get("item", "")),
                    str(event.data.get("raw_line", "")),
                )
            ).casefold()
        ...
        # Xonotic also emits carrierfrag for friendly-carrier kills.
        # Those events use zero or negative player points and must not
        # receive a positive Key Hunt carrier-kill reward.
        if (
            event.type == "keyhunt_event"
            and str(
                event.data.get("action", "")
            ).casefold() == "carrierfrag"
        ):
            try:
                player_points = float(
                    event.data.get("player_points")
                )
            except (TypeError, ValueError):
                player_points = None

            if (
                player_points is not None
                and player_points <= 0.0
            ):
                return RewardLedger()
