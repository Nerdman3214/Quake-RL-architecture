"""Two-arm fixed-step diagnostic ablation. Audit first; no production promotion.

Only explicit --run executes optimizers, after a pinned preflight and pushed code.
The one-use output directory and exclusive arm records forbid automatic retries.
"""
...
BATCH = 4
STEPS = 33790
SCHEDULE = list(range(3379, STEPS + 1, 3379))
OLD = ['V4', 'V6', 'V7', 'V9', 'V10']
FULL = OLD + ['V11b']
ARMS = {'old_only': OLD, 'old_plus_v11b': FULL}
...
def prepare():
    ...
    require(len(history) == 10 and math.ceil(manifest['train_sequence_count'] / BATCH) * len(history) == STEPS,
            'Production horizon changed')
...
def preflight():
    ...
    a = seed_model(torch, v2)
    ha = state_hash(a)
    b = seed_model(torch, v2)
    hb = state_hash(b)
    require(ha == hb, 'Initial model states differ; stop before optimizer')
    exposures = {}
    for arm, names in ARMS.items():
        sessions = {}
        for name in names:
            labels = datasets[name].labels
            p = sum(l.jump for l in labels) / len(labels) / len(names)
            left = sum(l.strafe == 0 for l in labels) / len(labels) / len(names)
            sessions[name] = dict(session_probability=1/len(names),
                                  jump_positive_probability=p, expected_jump_draws=STEPS*BATCH*p,
                                  left_probability=left, expected_left_draws=STEPS*BATCH*left)
        exposures[arm] = dict(sessions=sessions,
                             jump_probability=sum(r['jump_positive_probability'] for r in sessions.values()),
                             expected_old_jump_draws=sum(r['expected_jump_draws'] for n,r in sessions.items() if n in OLD),
                             expected_v11b_jump_draws=sessions.get('V11b', {}).get('expected_jump_draws', 0))
    return dict(preflight_passed=True, runner_sha256=sha(Path(__file__)),
                contract_sha256=CONTRACT_SHA, source_pins=PINS, seed=SEED,
                initial_state_hashes=dict(old_only=ha, old_plus_v11b=hb),
                architecture=v2.architecture_metadata(), parameter_count=sum(p.numel() for p in a.parameters()),
                optimizer=dict(name='AdamW', learning_rate=5e-5, weight_decay=.0005,
                               betas=[.9,.999], eps=1e-8, amsgrad=False, gradient_clip_norm=1.0),
                loss_configuration=loss_values(config),
                loss_coefficients=dict(forward=1., strafe=1., jump=.75, yaw_active=.5,
                                       pitch_active=.5, yaw_regression=.25, pitch_regression=.25),
                arms=ARMS, counts=COUNTS, steps=STEPS, evaluation_steps=SCHEDULE, batch_size=BATCH,
                samples_per_arm=STEPS*BATCH, exposures=exposures,
                horizon_basis='Completed production: 10 * ceil(13514/4) = 33790 updates',
                batching='Continuous replacement draws; every update has four samples, no partial epoch batch',
                production_difference='135160 rather than 135140 sample draws; 20 extra samples at the same update budget',
                sampler='equal_session_probability_mass_with_replacement', event_oversampling=False,
                early_stopping=False, retries=False, diagnostic_only=True, training_started=False, **FLAGS)
...
def run_arm(arm, directory, audit):
    ...
    require(initial == audit['initial_state_hashes'][arm], 'Initial-state mismatch')
...
def execute(output, audit_path, audit_sha):
    saved = json.loads(pinned(audit_path, audit_sha).read_text())
    require(saved['preflight_passed'] is True and saved['runner_sha256'] == sha(Path(__file__)), 'Stale preflight')
    fresh = preflight()
    require(fresh == saved, 'Preflight changed')
    head = subprocess.check_output(['git','rev-parse','HEAD'], cwd=ROOT,text=True).strip()
    remote = subprocess.check_output(['git','rev-parse','origin/main'], cwd=ROOT,text=True).strip()
    require(head == remote, 'Code checkpoint not pushed')
    committed = subprocess.check_output(['git','show',f'{head}:src/RL/visual_bc/run_v11b_transfer_ablation_v1.py'],cwd=ROOT)
    require(hashlib.sha256(committed).hexdigest() == saved['runner_sha256'], 'Runner not committed')
    require(not output.exists(), 'Refusing an existing experiment directory')
    output.mkdir(parents=False, exist_ok=False)
    write_new(output/'preflight.json', saved)
    write_new(output/'experiment_started.json', dict(commit=head, steps=STEPS, arms=ARMS, no_retries=True, **FLAGS))
    all_rows = {}; results = {}
    try:
        for arm in ARMS:
            all_rows[arm], results[arm] = run_arm(arm, output/arm, saved)
        deltas = []
        for a,b in zip(all_rows['old_only'], all_rows['old_plus_v11b']):
            require(a['global_step'] == b['global_step'], 'Evaluation schedule mismatch')
            old, full = a['V5'], b['V5']
            deltas.append(dict(global_step=a['global_step'], **{
                'jump_'+k:full['jump'][k]-old['jump'][k]
                for k in ['AP','ROC_AUC','positive_negative_gap','best_diagnostic_F1']},
                left_F1=full['strafe']['left_F1']-old['strafe']['left_F1'],
                forward_F1=full['forward']['macro_F1']-old['forward']['macro_F1'],
                yaw_AP=full['yaw']['AP']-old['yaw']['AP'], pitch_AP=full['pitch']['AP']-old['pitch']['AP']))
        write_new(output/'comparison.json', dict(results=results, V5=all_rows, full_minus_old=deltas, **FLAGS))
    except BaseException as exc:
        write_new(output/'experiment_failure.json', dict(error=repr(exc), automatic_retry=False, **FLAGS))
        raise
