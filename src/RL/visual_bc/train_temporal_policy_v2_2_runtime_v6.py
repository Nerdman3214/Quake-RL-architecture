...
def evaluate(
    ...
    yaw_active_magnitude_examples = int(
        yaw_mask.sum().item()
    )

    pitch_active_magnitude_examples = int(
        pitch_mask.sum().item()
    )
    ...
    result.update(
        {
            ...
            "yaw_active_magnitude_examples":
                yaw_active_magnitude_examples,

            "pitch_active_magnitude_examples":
                pitch_active_magnitude_examples,

            "yaw_magnitude_penalty":
                yaw_penalty,

            "pitch_magnitude_penalty":
                pitch_penalty,
...
def make_datasets_v2_2(
    ...
    if expected_train_count != 13514:

        raise RuntimeError(
            "V2.2 contract does not specify "
            "13514 training sequences."
        )
